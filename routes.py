...
def admin_summary_data():
    ...
    for subject in subjects:
        label.append(subject.subjectname)
        count = 0
        for quiz in quizzes:
            if quiz.subjectid == subject.subjectid:
                count += 1
        data.append(count)
    chartData.append([label, data])
    
    label, data = [], []
    for chap in chapters:
        label.append(chap.chaptername)
        count = 0
        for question in questions:
            if question.chapterid == chap.chapterid:
                count += 1
        data.append(count)
    chartData.append([label, data])

    label, data = ['0-25', '26-50', '51-75', '76-100'], [0, 0, 0, 0]
    for user in users:
        totalScore = 0
        noOfTests = 0
        for score in scores:
            if user.id == score.userid:
                value = (score.correctanswers / score.totalquestions) * 100
                totalScore += value
                noOfTests += 1
        if noOfTests == 0:
            data[0] += 1
        else:
            avg = totalScore / noOfTests
            if avg <= 25:
                data[0] += 1
            elif avg <= 50:
                data[1] += 1
    ...
    label, data = ['Class 10', 'Class 12', 'Bacholors', 'Masters', 'PhD'], [0, 0, 0, 0, 0]
    for user in users:
        if user.qualification == '10':
            data[0] += 1
        elif user.qualification == '12':
            data[1] += 1
        elif user.qualification == 'bachelor':
            data[2] += 1
        elif user.qualification == 'master':
            data[3] += 1
        else:
            data[4] += 1
    chartData.append([label, data])
...
def user_summary(id):
    scores = Scores.query.filter_by(userid = id).all()
    quizzes = Quizzes.query.all()
    subjects = Subjects.query.all()
    ...
    label, data = [], []
    for quiz in quizzes:
        label.append(quiz.quizname)
        totalScore = 0
        noOfTests = 0
        for score in scores:
            if quiz.quizid == score.quizid:
                value = int((score.correctanswers / score.totalquestions) * 100)
                totalScore += value
                noOfTests += 1
        if noOfTests > 0:
            data.append(int(totalScore/noOfTests))
        else:
            data.append(0)
    chartData.append([label, data])
    ...
    label, data = [], []
    for sub in subjects:
        label.append(sub.subjectname)
        totalScore = 0
        noOfTests = 0
        for score in scores:
            if sub.subjectname == score.subject:
                value = int((score.correctanswers / score.totalquestions) * 100)
                totalScore += value
                noOfTests += 1
        if noOfTests > 0:
...
@login_required
def admin_summary():
    return render_template('admin-summary.html', user = current_user, data = admin_summary_data())


@app.route('/manage-users')
@login_required
def manage_users():
    return render_template('manage.html', user = current_user, users = User.query.filter_by(is_admin=False).all())
...
def delete_user():
    ...
    if user:
        scores = Scores.query.filter_by(userid=user_id).all()
        for score in scores:
...
@login_required
def summary():
    return render_template('summary.html', user = current_user, data = user_summary(current_user.id))
